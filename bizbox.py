...
class BizBoxPlaywright:
    ...
    def login(self, page):
        ...
        try:
            ...
            # 방법 2: submit 타입 input
            elif page.query_selector('input[type="submit"]'):
            ...
            # 방법 3: 텍스트로 찾기
                ...
                for btn in buttons:
                    ...
                    if '로그인' in text or 'Login' in text or '로그' in text:
                        login_button = btn
                        print(f"Found login button by text: {text}")
                        break
            
            if not login_button:
                print("Warning: Login button not found, attempting click on common selector...")
                page.click('button[type="submit"], input[type="submit"], button:has-text("로그인")', timeout=5000)
            else:
                login_button.click()
